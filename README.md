...
## 2.2 Optional: Fetch Covers from IGDB API

You can auto-populate game folders with official covers from IGDB.

1. Create API credentials:
- Twitch/IGDB `client_id`
- Twitch/IGDB `client_secret`
...
3. (Optional) Fill mapping file for ambiguous game names:
- `src/igdb_game_mappings.csv`
- Columns: `local_game,igdb_game_id,igdb_game_name`

4. Dry run first:

```bash
python src/fetch_igdb_covers.py --dry-run
```

5. Download covers:

```bash
python src/fetch_igdb_covers.py \
  --image-size cover_big \
  --mapping-csv src/igdb_game_mappings.csv
```

For stricter matching on slug-like folder names:

```bash
python src/fetch_igdb_covers.py \
  --strict-match-mode \
  --auto-clean-local-names \
  --min-match-score 0.72 \
  --min-token-overlap 0.34
```

To auto-create new folders from IGDB genre (example: Indie) and download covers:

```bash
python src/fetch_igdb_covers.py \
  --seed-genre-name Indie \
  --seed-count 30 \
  --seed-update-groups \
  --seed-group-label indie \
  --seed-write-mappings
```

To auto-create AAA-like games from 2010-2020 (popular, excluding IGDB Indie genre):

```bash
python src/fetch_igdb_covers.py \
  --seed-mode popular_year_range \
  --seed-count 30 \
  --seed-year-start 2010 \
  --seed-year-end 2020 \
  --seed-min-total-rating-count 120 \
  --seed-exclude-genre-name Indie \
  --seed-update-groups \
  --seed-group-label aaa \
  --seed-write-mappings
```

To add specific IGDB games by ID:

```bash
python src/fetch_igdb_covers.py \
  --seed-mode id_list \
  --seed-game-ids 1020,1942,7346 \
  --seed-group-label aaa \
  --seed-update-groups \
  --seed-write-mappings
```

To search IGDB titles from terminal:

```bash
python src/fetch_igdb_covers.py --search-query "Elden Ring" --search-limit 20
```

To search by company (and optionally combine with title filter):

```bash
python src/fetch_igdb_covers.py --search-company "Rockstar" --search-limit 20
python src/fetch_igdb_covers.py --search-query "Red Dead" --search-company "Rockstar" --search-limit 20
```

To add a list of game names (one per line) from a text file:

```bash
python src/fetch_igdb_covers.py \
  --seed-mode name_list \
  --seed-game-list-file path/to/game_list.txt \
  --seed-group-label aaa \
  --seed-update-groups \
  --seed-write-mappings
```

Or pass names inline:

```bash
python src/fetch_igdb_covers.py \
  --seed-mode name_list \
  --seed-game-names $'Hades\nControl\nRed Dead Redemption 2' \
  --seed-group-label aaa \
  --seed-update-groups \
  --seed-write-mappings
```

The script writes a report to `web/data/igdb_cover_fetch_report.json`.

If you see SSL certificate verification errors on macOS:
...
- `Add AAA 2010-2020` to auto-create 30 AAA-like game folders (popular, release window filtered, excluding IGDB Indie) and fetch their covers.
- Use `Search IGDB Games` with optional `Company` filter, select one or many results, then click `Add Selected Games`.
- Use `Add Game List` + `Add Full List` to paste many titles and import them in one run.
- Use `Import PDF List` to upload a PDF, extract game names, and import them in one run.
